...
export function describeHand(evaluation: HandEvaluation): string {
  ...
  switch (category) {
    case 'High Card':
      return `High Card (${rankName(ranks[0])})`;
    case 'Pair':
      return `Pair (${rankName(ranks[0])})`;
    case 'Two Pair':
      return `Two Pair (${rankName(ranks[0])} + ${rankName(ranks[1])})`;
    case 'Three of a Kind':
      return `Three of a Kind (${rankName(ranks[0])})`;
    case 'Straight':
      return `Straight (${rankName(ranks[0])} high)`;
    case 'Flush':
      return `Flush (${rankName(ranks[0])} high)`;
    case 'Full House':
      return `Full House (3${rankName(ranks[0])} + 2${rankName(ranks[1])})`;
    case 'Four of a Kind':
      return `Four of a Kind (${rankName(ranks[0])})`;
    case 'Straight Flush':
      return ranks[0] === 14 ? 'Royal Flush' : `Straight Flush (${rankName(ranks[0])} high)`;
  }
}

/**
 * When two hands share a category, describes the rank that decided between
 * them (e.g. "K kicker"), for showing how a close hand was won.
 * Returns null if the hands are fully tied (true split).
 */
...
/** Category-ordinal strength bar. Verdict labels are deliberately editable. */
...
/**
 * Preflop-only strength bar based on the two hole cards, using a variant of
 * the Chen Formula (pair/suited/connector bonuses, gap penalties). Hand
 * Category alone can't distinguish AA from 72o (both "Pair"/"High Card"), but
 * starting-hand quality varies hugely, so preflop gets its own scale.
 */
export function preflopStrength(cards: Card[]): StrengthBar {
  ...
  if (isPair) {
    ...
    if (gap <= 1 && hi <= 12) score += 1; // straight potential, both Q-or-lower
  }
...
/**
 * Predetermined board-texture concern lines, based only on community cards
 * vs. the player's current hand category (see CONTEXT.md: Board Texture Concern).
 */
export function boardConcerns(evaluation: HandEvaluation, communityCards: Card[]): string[] {
  ...
  const flushSuit = [...suitCounts.entries()].find(([, count]) => count >= 3)?.[0];
  if (flushSuit && evaluation.categoryIndex < 5) {
    concerns.push('Note: the board allows a possible flush.');
  }

  const communityValues = [...new Set(communityCards.map((c) => rankValue(c.rank)))].sort((a, b) => a - b);
  let hasRunOfThree = false;
  for (let i = 0; i <= communityValues.length - 3; i++) {
    if (communityValues[i + 2] - communityValues[i] <= 4) {
      hasRunOfThree = true;
      break;
    }
  }
  ...
  const communityRankCounts = new Map<number, number>();
  for (const card of communityCards) {
    const v = rankValue(card.rank);
    communityRankCounts.set(v, (communityRankCounts.get(v) ?? 0) + 1);
  }
  const boardPaired = [...communityRankCounts.values()].some((count) => count >= 2);
  if (boardPaired && evaluation.categoryIndex < 6 && evaluation.categoryIndex !== 7) {
