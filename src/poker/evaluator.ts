...
export interface HandEvaluation {
  ...
  ranks: number[];
  /** The cards (subset of the input) that make up this hand, for highlighting. */
  cards: Card[];
  /**
   * The subset of `cards` that defines the category itself (e.g. the pair,
   * trips, or both pairs in Two Pair) as opposed to kickers. For categories
   * where all 5 cards define the hand (straights, flushes, full houses), this
   * equals `cards`. Empty for High Card.
   */
  comboCards: Card[];
}
...
/**
 * Compares two evaluations. Returns >0 if a beats b, <0 if b beats a, 0 if tied.
 */
...
/**
 * Evaluates the best hand from 2-7 cards. With fewer than 5 cards (preflop),
 * falls back to a simple Pair/High Card read on the available cards.
 */
...
function evaluate5(cards: Card[]): HandEvaluation {
  ...
  // Groups of same-rank cards, sorted by (count desc, value desc).
  ...
  if (isFlush && straightHigh !== null) {
    const cards = pickStraightCards(sorted, straightHigh);
    return { category: 'Straight Flush', categoryIndex: 8, ranks: [straightHigh], cards, comboCards: cards };
  }
  ...
  if (groups[0].group.length === 3 && groups[1].group.length >= 2) {
    const cards = [...groups[0].group, ...groups[1].group.slice(0, 2)];
    return { category: 'Full House', categoryIndex: 6, ranks: [groups[0].value, groups[1].value], cards, comboCards: cards };
  }
  ...
  if (straightHigh !== null) {
    const cards = pickStraightCards(sorted, straightHigh);
    return { category: 'Straight', categoryIndex: 4, ranks: [straightHigh], cards, comboCards: cards };
  }
...
/** Returns the high card of a straight among the given values, or null. Handles the wheel (A-2-3-4-5). */
...
function pickStraightCards(sorted: Card[], high: number): Card[] {
  const wanted = straightRanks(high);
  const result: Card[] = [];
...
function straightRanks(high: number): number[] {
  if (high === 5) return [5, 4, 3, 2, 14]; // wheel: 5-4-3-2-A
